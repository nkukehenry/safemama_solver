...
<center><img onclick="$('#photo').click();" class="profile-user-img img-responsive img-circle userphoto" src="<?php echo base_url(); ?>assets/images/users/<?php echo $userdata['photo']; ?>" data-toggle="tooltip" title="Click to change photo" style="border:8px grey solid; max-width: 130px;"></center>
          ...
          <div class="panel">
            <div class="panel-body">
                ...
                <form class="form-horizontal" class="profile" method="post" action="<?php echo base_url();?>auth/updateProfile">
                  ...
                  <div class="form-group">
                <label class="text-black">My School</label>
                <select name="school_id" class="form-control select2" required placeholder="Name of your school">
                    
                  <?php  if(!$userdata['school_id']): ?>
                  <option value="0">None</option>
                  <?php endif; ?>
                  <?php foreach ($schools as $row):
                       $selected ="";
                    if($row->school_id==$userdata['school_id']){
                        $selected ="selected";
                    }
                  ?>
                    <option value="<?=$row->schoolname?>" <?=$selected?>><?=$row->schoolname?></option>
                  <?php endforeach; ?>
      ...
      <!-- /.big col 8 -->
        ...
        <div class="col-md-4" >
          ...
          $mycourses=Modules::run("courses/getUserCourses",10,0,$userdata['user_id']);
